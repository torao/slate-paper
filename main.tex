...
Slate は History Tree \cite{crosby2009} に基づいた追記操作が可能な Merkle Tree \cite{merkle1989} の亜種であり、その二分木の葉ノードをそれぞれ 1 つのデータに対応させることで順序付きの列を表現する。本論文では $n$ 個の葉ノード (つまり $n$ 個のデータ) で構成される木構造を $T_n$ と表記し、特に $T_n$ の成長過程に注目するとき $n$-世代の木と表現する。ここで $T_0 = \emptyset$ である。便宜上、あるノード $b$ が木構造 $T$ に含まれていることを $b \in T$ と表記し、$T_1$ が $T_2$ の部分木であることを $T_1 \subseteq T_2$ と表記する。
...
\label{eq:ij}
\end{equation}
$j=0$ は葉ノードのケースであり、$b_{i,0}=b_i$ と表す。また、$T_n$ に含まれる任意のノード $b_{i,j}$ をルートとする部分木を $T_{i,j} \subseteq T_n$ と表す。

\textbf{完全二分木}: 本論文は木構造が完全二分木 (perfect binary tree) であることを強調するために記号 $'$ を用いる。$T'_n$ は $n$ 世代目の木構造が完全二分木であることを意味する。また $T'_{i,j}$ は部分木 $T_{i,j}$ が完全二分木であることを意味し、このとき $T'_{i,j}$ のルートを $b'_{i,j}$ と表す。

\textbf{部分的永続データ構造}: Slate の木は部分的永続データ構造 \cite{driscoll1986} (partially persisten data structure) である。$T_n$ に含まれる任意の完全二分部分木のルートノード $b'_{i,j}$ は永続性 (persistent) を持つ。つまり、$m > n$ となるような $n$ 以降のすべての世代の木 $T_m$ においても同じノード $b'_{i,j}$ が存在し続ける。ノード $b_{i,j}$ をルートとする部分木が完全二分木ではない場合、$b_{i,j}$ はその世代に限り存在する一過性 (ephemeral) であり、それ以降の世代では木構造に現われることはない。
...
\end{equation}
すべての葉ノード $b_{i,0}$ は $i \bmod{2^0} = 0$ であることから永続性ノードである。

\textbf{エントリ}: $i$ 番目のデータ $d_i$ を木に追加するときに発生する追加の構造を\textbf{エントリ} (entry) と呼び $e_i$ で表す。$e_i$ はデータ $d_i$ とそれを木と結合するノード集合 $\{b_i,b_{i,j_1},b_{i,j_2},\ldots\}$ で構成されている。$e_i$ に含まれるノード $b_{i,j}$ の添字 $i$ はすべて同じであるため、ここでは $e_i$ を構成するノードの添字 $j$ を求める。
...
\end{eqnarray}
$\mathcal{J}'$ と $\mathcal{J}$ はそれぞれ永続性ノードと一過性ノードに割り当てられる添字 $j$ を区別している。これより、エントリ $e_i$ は式 (\ref{eq:eij}) のように表すことができる。
\begin{equation}
    e_i = \{ b'_{i,j} \mid j \in \mathcal{J}'(i) \} \cup \{b_{i,j} \mid j \in \mathcal{J}(i)\}
    \label{eq:eij}
\end{equation}
\textbf{木構造の構築}: Slate は、独立した (互いに部分構造ではない) 完全二分木を一世代限りの一過性ノードで接続して決定論的な木を構築する。図 \ref{fig:t22} は $n=22$ の木構造 $T_{22}$ を表しており、3 つの完全二分部分木 $T'_{16,4}, T'_{20,2}, T'_{22,1}$ を 2 つの一過性ノード $b_{22,5},b_{22,3}$ で接続して木を成している様子を表している。

$T_n$ に含まれる完全二分部分木の数は $n$ の 2 進数表現での 1 の個数 $\mathrm{popcount}(n)$ に等しく、ぞれぞれの完全二分部分木の高さ $j$ は、$n$ の 2 進数表現において対応する 1 が現われる位置と等しい (最右ビットの位置を 0 とする)。高さ $j$ の完全二分木は $2^j$ 個の葉ノードを含んでいることから、$n$ は式 (\ref{eq:n}) のように各完全二分部分木の葉ノード数の和で表すことができる。
\begin{equation}
n = \sum_{j \ge 0} \alpha_j\,2^j \quad (\alpha_j \in \{0,1\})
\label{eq:n}
\end{equation}
ここで $\alpha_j$ は $n$ の 2 進数表現の $j$ 桁目の値を意味する。これより $T_n$ に含まれる完全二分部分木の各ルートノードの集合 $\mathcal{B}'_n$ は式 \ref{eq:pbst_roots} のように表すことができる。
\begin{equation}
\mathcal{B}'_n = \left\{b_{i,j},i=\left\lfloor\frac{n}{2^j}\right\rfloor\cdot2^j\,\left|\, j \in \mathbb{N}_0,  
  \left\lfloor\frac{n}{2^j}\right\rfloor \bmod{2} = 1 \right.\right\} 
\label{eq:pbst_roots}
\end{equation}
\textbf{ノードの接続}: ここで、$T_n$ に含まれる任意の非葉ノード $b_{i,j}, j > 0$ に対して、$\mathrm{left(i,j)}$ を左の子ノードの添字を得る関数、$\mathrm{right(i,j)}$ を右の子ノードの添字を得る関数とする。まず、$T_n$ に含まれる任意の非葉ノード $b_{i,j}$ に対して左の子ノードは式 (\ref{eq:child_left}) で求めることができる。
\begin{equation}
    \mathrm{left}(i,j) = \left(\left\lfloor \frac{i-1}{2^{j-1}} \right\rfloor \cdot 2^{j-1},j - 1\right)
    \label{eq:child_left}
\end{equation}
右のノードは $b_{i,j}$ が永続性ノードか一過性ノードかで求め方が異なる。一過性ノードの場合、接続の変則性から直下の $\mathcal{J}(i)$ または $\mathcal{J}'(i)$ の頂点と接続する。
\begin{equation}
    \textrm{right}(i,j) = \begin{cases}
        (i, j-1) & \text{if $b_{i,j}$ is persistent} \\
        \left(i,
        \begin{cases}
            \mathrm{ctz}(i) & \text{if $j = \min \mathcal{J}(i)$} \\
            \max\{k \in \mathcal{J}(i) \mid k < j\} & \text{otherwise}
        \end{cases}
        \right) & \text{otherwise, it is ephemeral}
    \end{cases}
\end{equation}
任意の非葉ノード $b_{i,j}$ とその右の子ノードは同じエントリ $e_i$ に属することから、任意のノードの右の子ノードの $i$ 成分は常に $b_{i,j}$ と同じ値である。

\textbf{例 1}: 図 \ref{fig:t22} の例は上記の説明を視覚的に表している。$T_{22}$ における $n=22$ の 2 進数表現 \texttt{10110} から、$T_{22}$ には $\mathcal{B}'_{22}=\{b'_{16,4},b'_{20,2},b'_{22,1}\}$ をルートとする 3 つの完全二分部分木が含まれており、それらを連結する一過性ノード $\{b_{22,5},b_{22,3}\}$ によって単一の木構造を成している。$T_{22}$ において $i=22$ を持つノードはエントリ $e_{22}$ を構成している。
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=.6]{fig/t22-structure-sketch.pdf}
  \caption{$T_{22}$ の木構造: 2進数表現と完全二分部分木の対応}
  \label{fig:t22}
\end{figure}
\textbf{直列化形式}: 木構造 $T_n$ の直列化形式はエントリの列 $S_n=(e_1,e_2,\ldots,e_n)$ として表される。
図 \ref{fig:t6_to_t7} は $T_6$ から $T_7$ への木構造の遷移と、直列化形式の変化を示している。

$T_n$ に含まれる完全二分部分木の集合を $\mathcal{T}'_n = \{T'_{i,j}\}$ 
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=.8]{fig/t6-to-t7.pdf}
  \caption{$T_{6}$ から $T_7$ への遷移}
  \label{fig:t6_to_t7}
\end{figure}
